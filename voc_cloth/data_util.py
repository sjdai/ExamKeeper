...
class ClothSample(object):
    ...
    def convert_tokens_to_ids(self, tokenizer):
        ...
        for i in range(len(self.ops)):
            for k in range(len(self.ops[i])): # for k in range(4):
...
class Preprocessor(object):
    def __init__(self, bert_model, tokenizer, data, device='cpu'):
...
class Loader(object):
    ...
    def _batchify(self, data_set, data_batch):
        ...
        options = torch.zeros(bsz, max_ops_num, len(data_set[0].ops[0]), max_option_length).long()
...
if __name__ == '__main__':
    ...
    for item in data_collections:    
        ...
        preprocessor = Preprocessor(args.bert_model, tokenizer, all_data)
        
        ##########################remove from Preprocessor###################################
        torch.save(preprocessor.data_objs, args.save_name)
        #####################################################################################
